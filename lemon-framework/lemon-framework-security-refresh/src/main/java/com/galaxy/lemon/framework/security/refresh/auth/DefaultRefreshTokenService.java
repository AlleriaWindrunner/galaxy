...
import com.galaxy.lemon.framework.security.refresh.AccessToken;
...
public class DefaultRefreshTokenService implements RefreshTokenService {
    ...
    public DefaultRefreshTokenService(TokenService tokenService) {
        this.tokenService = tokenService;
    }

    @Override
    public String resolveRefreshToken(HttpSession httpSession) {
        AccessToken accessToken = this.tokenService.createAccessToken(httpSession);
        return accessToken.getRefreshToken().getValue();
    }

    @Override
    public String extractRefreshToken(HttpSession httpSession) {
        return tokenService.getAccessToken(httpSession).getRefreshToken().getValue();
    }

    @Override
    public void revokeRefreshToken(String sessionId) {
        if (StringUtils.isNotBlank(sessionId)) {
            tokenService.revokeToken(null, sessionId);
        }
    }
}
