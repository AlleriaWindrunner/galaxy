...
public class RetryableExceptionConverter implements ExceptionConverter {
    @Override
    ...
    public AlertCapable convert(Throwable throwable) {
        if(ExceptionConverter.isTimeOutException(throwable.getCause())) {
            return ErrorMsgCode.CLIENT_TIMEOUT;
        }
        if(null != throwable.getCause() && throwable.getCause() instanceof UnknownHostException) {
            return ErrorMsgCode.CLIENT_EXCEPTION_UNKNOWN_HOST;
        }
        return ErrorMsgCode.CLIENT_EXCEPTION;
    }
}
