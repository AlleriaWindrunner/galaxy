...
public class IdGeneratorExclusiveRedisCondition extends SpringBootCondition {
    ...
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        RelaxedPropertyResolver resolver = new RelaxedPropertyResolver(context.getEnvironment(), "lemon.idgen.redis.");
        ...
        if (JudgeUtils.isBlank(url)
                ...
                && (JudgeUtils.isEmpty(sentinelConfig) || !sentinelConfig.containsKey("nodes"))
                && (JudgeUtils.isEmpty(clusterConfig) || !clusterConfig.containsKey("nodes"))) {
            return ConditionOutcome.noMatch(
                    message.didNotFind("lemon.idgen.redis property").items("url", "host", "sentinel", "cluster"));
        }

        return ConditionOutcome.match(message.because("one of property 'lemon.idgen.redis.url/host/sentinel/cluster' found. "));
    }
