 ...
 */
package com.galaxy.lemon.common.bytecode;
...
public final class ClassGenerator {
    ...
    public ClassGenerator addField(String name, int mod, Class<?> type, String def) {
        ...
        sb.append(modifier(mod)).append(' ').append(JavassistHelper.getName(type)).append(' ');
        sb.append(name);
    ...
    public ClassGenerator addMethod(String name, int mod, Class<?> rt, Class<?>[] pts, Class<?>[] ets, String body) {
        StringBuilder sb = new StringBuilder();
        sb.append(modifier(mod)).append(' ').append(JavassistHelper.getName(rt)).append(' ').append(name);
        sb.append('(');
        for (int i = 0; i < pts.length; i++) {
            if (i > 0)
                sb.append(',');
            sb.append(JavassistHelper.getName(pts[i]));
        ...
        if (ets != null && ets.length > 0) {
            ...
            for (int i = 0; i < ets.length; i++) {
                if (i > 0)
                    sb.append(',');
                sb.append(JavassistHelper.getName(ets[i]));
            }
        }
    ...
    public ClassGenerator addMethod(String name, Method m) {
        String desc = name + JavassistHelper.getDescWithoutMethodName(m);
        addMethod(':' + desc);
    ...
    public ClassGenerator addConstructor(Constructor<?> c) {
        String desc = JavassistHelper.getDesc(c);
        addConstructor(":" + desc);
    ...
    private CtMethod getCtMethod(Method m) throws NotFoundException {
        return getCtClass(m.getDeclaringClass()).getMethod(m.getName(), JavassistHelper.getDescWithoutMethodName(m));
    }

    private CtConstructor getCtConstructor(Constructor<?> c) throws NotFoundException {
        return getCtClass(c.getDeclaringClass()).getConstructor(JavassistHelper.getDesc(c));
    }
